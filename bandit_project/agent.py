...
class NaiveEpsilonGreedyAgent(Agent):
    def __init__(self, epsilon, **kwds):
        self.epsilon = epsilon
    ...
    def pull_arm(self):
        ...
        # Get greedy with probability (1 - epsilon)
            ...
            I = np.argmax(self.theta_hat_array) + 1
        reward = correlated_machine.get_reward(I)
    ...
    def name(self):
        return 'Naive $\\epsilon$-greedy with $\\epsilon = {}$'.format(self.epsilon)


class CorrelatedEpsilonGreedyAgent(Agent):
    def __init__(self, epsilon, **kwds):
        self.epsilon = epsilon
    ...
    def pull_arm(self):
        ...
        if I in [1, 2]:
            I1 = 1 + 2 - I
            self.count_array[I1-1] += 1
            self.theta_hat_array[I1-1] += (reward - self.theta_hat_array[I1-1]) / self.count_array[I1-1]

        return I, reward, None


    def name(self):
        return 'Correlated $\\epsilon$-greedy with $\\epsilon = {}$'.format(self.epsilon)
...
class NaiveTsAgent:
    ...
    def name(self):
        return 'Naive TS with $\{{(\\alpha_1, \\beta_1) = ({}, {}), (\\alpha_2, \\beta_2) = ({}, {}), (\\alpha_3, \\beta_3) = ({}, {})\}}$'.format(self.initial_alpha_list[0], self.initial_alpha_list[1], self.initial_alpha_list[2], self.initial_beta_list[0], self.initial_beta_list[1], self.initial_beta_list[2])


class CorrelatedTsAgent:
    def __init__(self, alpha_list, beta_list, **kwds):
        self.initial_alpha_list = copy.deepcopy(alpha_list)
        self.initial_beta_list = copy.deepcopy(beta_list)
    ...
    def pull_arm(self):
        ...
        if I in [1, 2]:
            I1 = 1 + 2 - I
            self.alpha_list[I1 - 1] += reward
            self.beta_list[I1 - 1] += 1 - reward

        return I, reward, (copy.deepcopy(self.alpha_list), copy.deepcopy(self.beta_list))

    
    def name(self):
        return 'Correlated TS with $\{{(\\alpha_1, \\beta_1) = ({}, {}), (\\alpha_2, \\beta_2) = ({}, {}), (\\alpha_3, \\beta_3) = ({}, {})\}}$'.format(self.initial_alpha_list[0], self.initial_alpha_list[1], self.initial_alpha_list[2], self.initial_beta_list[0], self.initial_beta_list[1], self.initial_beta_list[2])
